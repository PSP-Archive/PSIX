...
void bilinear_resize(u32 *line1, u32 *line2, u32 *new_line, int new_width, double xfactor, 
		     double fraction_y, double one_minus_y) {
  int x, floor_x, ceil_x;
  double fraction_x, one_minus_x;
  u32 c1, c2, c3, c4, new_color, b1, b2;

  for(x=0 ;x < new_width ;x++ ) {
    floor_x = (int)(x * xfactor);
    ceil_x = floor_x + 1;
    ...
    /*red*/
    b1 = one_minus_x * (c1 & 0x00ff0000) + fraction_x * (c2 & 0x00ff0000);
    b2 = one_minus_x * (c3 & 0x00ff0000) + fraction_x * (c4 & 0x00ff0000);
    new_color = (u32)(one_minus_y * (double)(b1) + fraction_y * (double)(b2)) & 0x00ff0000;
    /*green*/
    b1 = one_minus_x * (c1 & 0x0000ff00) + fraction_x * (c2 & 0x0000ff00);
    b2 = one_minus_x * (c3 & 0x0000ff00) + fraction_x * (c4 & 0x0000ff00);
    new_color += (u32)(one_minus_y * (double)(b1) + fraction_y * (double)(b2)) & 0x0000ff00;
    /*blue*/
    b1 = one_minus_x * (c1 & 0x000000ff) + fraction_x * (c2 & 0x000000ff);
    b2 = one_minus_x * (c3 & 0x000000ff) + fraction_x * (c4 & 0x000000ff);
    new_color += (u32)(one_minus_y * (double)(b1) + fraction_y * (double)(b2)) & 0x000000ff;
    /*alpha*/
    b1 = one_minus_x * (c1 & 0xff000000) + fraction_x * (c2 & 0xff000000);
    b2 = one_minus_x * (c3 & 0xff000000) + fraction_x * (c4 & 0xff000000);
    new_color += (u32)(one_minus_y * (double)(b1) + fraction_y * (double)(b2)) & 0xff000000;

    new_line[x]=new_color;
  }
...
int sdi_load_png(char *name, char* filename)
{
  ...
  if (width > 512 || height > 512) {
    ...
    int floor_y, i=0;
    double xfactor = (double)width/(double)new_width;
    double yfactor = (double)height/(double)new_height;
    double fraction_y, one_minus_y;

    line1 = malloc(width*4);
    line2 = malloc(width*4);
      

    image = malloc(new_width*new_height*4);

    for(y=0 ;y < new_height ;y++ ) {
      floor_y = (int)(y * yfactor);
      fraction_y = y * yfactor - floor_y;
      one_minus_y = 1.0 - fraction_y;

      /*skip rows if theyre not needed*/
      for(i ;i < floor_y; i++) {
	png_read_row(png_ptr, NULL, png_bytep_NULL);
      }
      if(i == floor_y) {
	png_read_row(png_ptr, (u8*) line1, png_bytep_NULL); i++; }
      png_read_row(png_ptr, (u8*) line2, png_bytep_NULL); i++;

      bilinear_resize(line1, line2, &image[y*new_width], new_width, xfactor, fraction_y, one_minus_y);
      
      //line1 = line2;
      //memmove(line1,line2,width*4);
      u32 *tl=line1;
      line1 = line2;
      line2 = tl;
    }
    ...
    for (y = 0; y < height; y++) {
      png_read_row(png_ptr, (u8*) line1, png_bytep_NULL);
      for (x = 0; x < width; x++) {
	u32 color = line1[x];
